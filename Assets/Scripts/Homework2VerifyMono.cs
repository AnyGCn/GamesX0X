...
public class Homework2VerifyMono : MonoBehaviour
{
    ...
    {
        // set properties
        customMaterial.SetMatrix(GlobalConstant._ModelMatrixCustomShaderId, Coordinate3D.GetModelMatrix(target.transform));
        customMaterial.SetMatrix(GlobalConstant._ViewMatrixCustomShaderId, Coordinate3D.GetViewMatrix(observer.transform));
        customMaterial.SetMatrix(GlobalConstant._ProjectionMatrixCustomShaderId, Coordinate3D.GetProjectionMatrix(observer));
